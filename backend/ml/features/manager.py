...
FEATURES = [
    "days_since_mgr_change_own", "new_mgr_own",
    "days_since_mgr_change_opp", "new_mgr_opp",
]

NEW_MANAGER_DAYS = 35  # roughly the first five matches


def _asof_stint(df: pd.DataFrame, ctx: FeatureContext, team_col: str, prefix: str) -> pd.DataFrame:
    ...
    days = (
        (merged["kickoff_time"] - merged["stint_start"]).dt.days.reindex(order)
    )
